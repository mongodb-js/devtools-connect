...
import { Link, Icon, Description, MongoDBLogo, H3, KeylineCard, css, palette, spacing, cx, InlineCode, Body } from '@mongodb-js/compass-components';
...
function DocsLink({ productDocsLink, productName }: OIDCPageBaseProps): JSX.Element {
  return (productDocsLink ? <p><Link arrowAppearance="persist" href={productDocsLink}>{productName} Documentation</Link></p> : <></>);
}
    ...
    <body className={bodyStyles}>
      ...
      <div className={mainViewStyles}>
        <div className={mainContainerStyles}>
          <MongoDBLogo />
          <KeylineCard className={mainContentStyles}>
            {children}
          </KeylineCard>
        </div>
      </div>
...
export function OIDCAcceptedPage(baseProps: OIDCPageBaseProps): JSX.Element {
  return (<PageContainer title="Login Successful">
    <Icon glyph='CheckmarkWithCircle' size={40} className={cx(successIconStyles)} />
...
export function OIDCErrorPage({ error, errorDescription, errorURI, ...baseProps }: {
  error?: string, errorDescription?: string, errorURI?: string
} & OIDCPageBaseProps): JSX.Element {
  return (<PageContainer title="Authentication Failed">
    <Icon glyph='XWithCircle' size={40} className={cx(failureIconStyles)} />
    ...
    <Body>
      ...
      {(errorDescription || errorURI) && <Description>
        {errorDescription}

        {errorURI && <p><Link href={errorURI}>Additional information may be available here.</Link></p>}
      </Description>}
    </Body>
    <DocsLink {...baseProps} />
  </PageContainer>);

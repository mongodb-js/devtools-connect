import type { MongoDBOIDCPlugin, IdPServerInfo, IdPServerResponse, OIDCCallbackContext } from '@mongodb-js/oidc-plugin';
import type { DevtoolsConnectionState } from './connect';
import { Server as HTTPServer, IncomingMessage, ServerResponse, createServer as createHTTPServer, request } from 'http';
...
export async function readSignedStream(input: Readable, signature: string | string[] | undefined, hmacKey: Uint8Array): Promise<string> {
  ...
  let body = '';
  for await (const chunk of inputFromUtf8) { body += chunk; }
  await verifyFinish;
...
export function sign(payload: Uint8Array | string, hmacKey: Uint8Array): string {
...
export abstract class RpcServer {
  ...
  private async _handleRequest(req: IncomingMessage, res: ServerResponse) {
    let response;
    try {
      const content = JSON.parse(await readSignedStream(req,
...
export class RpcClient {
  ...
  public async makeRpcCall(reqData: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export class StateShareServer extends RpcServer {
  ...
  static async create(state: DevtoolsConnectionState): Promise<StateShareServer> {
...
export class StateShareClient extends RpcClient {
  ...
  private async _oidcCallback(method: string, info: IdPServerInfo, _context: OIDCCallbackContext): Promise<IdPServerResponse> {
    const timeoutContextId = (await promisify(crypto.randomBytes)(16)).toString('base64');

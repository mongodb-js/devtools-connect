...
import { sign, readSignedStream, RpcServer, RpcClient, StateShareServer, StateShareClient } from './ipc-rpc-state-share';
...
describe('IPC RPC state sharing', function() {
  ...
  describe('StateShareServer/StateShareClient', async function() {
    ...
    it('can be used to share OIDC state', async function() {
      ...
      const result = await client.oidcPlugin.mongoClientOptions.authMechanismProperties.REQUEST_TOKEN_CALLBACK({
    ...
    it('supports timeoutContext', async function() {
      refresh.callsFake((_idpInfo: unknown, ctx: { timeoutContext: AbortSignal }) => {
        return new Promise((resolve, reject) => ctx.timeoutContext.addEventListener('abort', () => reject(new Error('aborted'))));
      });
      server = await StateShareServer.create(fakeState);
      client = new StateShareClient(server.handle);

      const abortController = new AbortController();
      const result = client.oidcPlugin.mongoClientOptions.authMechanismProperties.REFRESH_TOKEN_CALLBACK({
        issuer: 'http://localhost/',
        clientId: 'clientId'
      }, {
        version: 0,
